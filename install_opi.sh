...
install_service_unit() {
  ...
  chown -R "$SERVICE_USER:$SERVICE_USER" "$BASE_DIR/workspace" "$BASE_DIR/logs" "$BASE_DIR/run" || true
  if [[ -f "$BASE_DIR/.env" ]]; then
  ...
  local template="$BASE_DIR/opi-agent.service.template"
...
AmbientCapabilities=
ReadWritePaths=$BASE_DIR/workspace $BASE_DIR/logs $BASE_DIR/run
